...
class ExcelExporter:
    def __init__(self, headers=None, field_types=None, text_columns=None, style_dict=None):
        """
        初始化ExcelExporter对象
        :param headers: 表头名称列表
        :param field_types: 字段类型字典, 例如: {'field1': str, 'field2': int}
        :param text_columns: 需要设置为文本格式的列名列表
        :param style_dict: 单元格样式字典, 例如: {'A1:B1': {'font': 'Arial', 'bold': True}}
        """
    ...
    def apply_styles(self, ws):
        ...
        :param ws: 工作表对象
        """
        for cell_range, styles in self.style_dict.items():
            for row in ws[cell_range]:
                for cell in row:
                    if 'font' in styles:
                        cell.font = Font(name=styles['font'])
                    if 'bold' in styles:
                        cell.font = Font(bold=styles['bold'])
                    if 'alignment' in styles:
                        cell.alignment = Alignment(horizontal=styles['alignment'])
...
if __name__ == '__main__':
    ...
    # 定义样式
    style_dict = {
        'A1:B1': {'font': 'Arial', 'bold': True, 'alignment': 'center'}
    }

    exporter = ExcelExporter(headers=headers, field_types=field_types, text_columns=text_columns, style_dict=style_dict)
    exporter.export(df, 'out.xlsx')
